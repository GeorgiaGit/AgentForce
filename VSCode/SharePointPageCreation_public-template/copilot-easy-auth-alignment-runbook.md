...
## This project: make it work and deploy (public template)

Use these steps for `SharePointPageCreation.Public.csproj` and function `CreateKbDraft_Public`.
...
```powershell
dotnet restore .\SharePointPageCreation.Public.csproj
dotnet build .\SharePointPageCreation.Public.csproj
```

### 3) Create Azure resources (example)
```powershell
...
### 5) Enable managed identity (required by `DefaultAzureCredential`)
```powershell
az functionapp identity assign --name <FUNCTION_APP_NAME> --resource-group <RESOURCE_GROUP>
```

### 6) Grant Graph + SharePoint permissions to the managed identity
The function calls Microsoft Graph to create/update SharePoint pages, so the managed identity needs Graph application permissions.

Minimum patterns:
- Broad access: `Sites.ReadWrite.All` (easier to validate quickly).
- Least privilege: `Sites.Selected` + explicit site grant for target site(s).

After assigning Graph app role permissions, ensure admin consent is granted.

### 7) Configure Easy Auth (for Copilot/connector scenarios)
Align authentication settings exactly as described earlier in this runbook:
- Provider client ID = `<API_APP_CLIENT_ID>`
- `allowedAudiences` includes `<API_IDENTIFIER_URI>`
- `allowedApplications` includes `<COPILOT_CLIENT_APP_ID>` (and optional trusted callers)
- Use `Return401` for production (use `AllowAnonymous` only as temporary troubleshooting)

### 8) Publish function code
...
1. Get function URL/key from Azure portal (or CLI) for `CreateKbDraft_Public`.
2. Update `payload.public.json` with your real target site/template path.
3. Invoke test script:
...
- Restrict `allowedApplications` to known caller app IDs only
- Use least-privilege Graph model (`Sites.Selected` where possible)
- Confirm no secrets or tenant-specific values are committed
...
| `<Env>` | Connector / API Tool | `<Workload>-Connector-<Env>` | `<CONNECTOR_NAME>` | Copilot Studio tool connection | Uses Copilot client app + API scope |
